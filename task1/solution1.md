...
![Minikube started using Docker driver](screenshots/minikube-started.png)

First, I tried to apply the original manifest to the cluster. And we get our first error:

![Yaml spacing error](screenshots/error-1.png)
...
![Yaml spacing fix](screenshots/fix-1.png)

I applied the file again, and the first error is gone, but we got another one:

![Naming error](screenshots/error-2.png)

There were two typos in the field names. I double-checked in the Kubernetes docs to get the correct spelling for `requiredDuringSchedulingIgnoredDuringExecution` and `targetPort`.

I fixed the typos and applied the file again. It works now:

![Naming fix](screenshots/fix-2.png)
...
![CPU error](screenshots/error-3.png)
![CPU error](screenshots/error-3-1.png)


I edited the deployment, so it needs less resources:

![CPU error](screenshots/fix-3.png)
...
![Service](screenshots/fix-5-1.png)
